...
function renderChart(history) {
  ...
  chart = new Chart($("#chart"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Total vehicles passed",
          data: values,
          borderColor: "#146df5",
          backgroundColor: "#146df51c",
          fill: true,
          tension: 0.25,
          pointRadius: 2,
          stepped: true,
...
async function refresh() {
  try {
    ...
    if ($("#vehicle-types")) {
      ...
      $("#vehicle-types").innerHTML = types.length
        ? types
            .map(
              ([name, count]) =>
                `<span class="vehicle-badge"><b>${name}</b><i>${count}</i></span>`,
            )
            .join("")
        : '<span class="empty-result">No vehicle detected</span>';
    }
    if ($("#passed-count"))
      $("#passed-count").textContent = state.passed_count || 0;
    if ($("#passed-types")) {
      ...
      $("#passed-types").innerHTML = passedTypes.length
        ? passedTypes
            .map(
              ([name, count]) =>
                `<span class="vehicle-badge passed"><b>${name}</b><i>${count}</i></span>`,
            )
            .join("")
        : '<span class="empty-result">No vehicle crossed the line</span>';
    }
    if ($("#density"))
      $("#density").textContent = `${Number(state.density).toFixed(1)}%`;
    if ($("#density-bar"))
      $("#density-bar").style.width = `${Math.min(100, state.density)}%`;
    if ($("#risk-percentage")) {
    ...
    if ($("#congestion")) {
      $("#congestion").textContent = state.congestion;
      $("#congestion").className = `level ${state.congestion.toLowerCase()}`;
    }
    ...
    if ($("#average"))
      $("#average").textContent = state.summary.average_vehicles;
    if ($("#avg-density"))
      $("#avg-density").textContent = `${state.summary.average_density}%`;
    if ($("#total-passed"))
      $("#total-passed").textContent = state.summary.total_passed || 0;
    if ($("#cars")) $("#cars").textContent = state.summary.cars || 0;
    if ($("#motorcycles"))
      $("#motorcycles").textContent = state.summary.motorcycles || 0;
    if ($("#buses")) $("#buses").textContent = state.summary.buses || 0;
    if ($("#trucks")) $("#trucks").textContent = state.summary.trucks || 0;
    if ($("#peak-risk"))
      $("#peak-risk").textContent = `${state.summary.peak_risk || 0}%`;
    $("#system-pill").className = `pill ${state.running ? "live" : "idle"}`;
    $("#system-pill").innerHTML =
      `<i></i> ${state.running ? "Analyzing" : "Offline"}`;
